...
const getProducts = (info) => {
  return new Promise((resolve, reject) => {
    let sqlQuery =
      "SELECT id, category_id, prod_name, price, image FROM products WHERE id <> 1";
    let parameters = " ";
    if (info.search) {
...
const getProductDetails = (info) => {
  return new Promise((resolve, reject) => {
    const sqlQuery =
      "SELECT p.id, prod_name, price, pc.category, image FROM products p join prod_categories pc ON p.category_id = pc.id WHERE p.id = $1";
...
const addProducts = (req) => {
  return new Promise((resolve, reject) => {
    const { body } = req;
    const sqlQuery =
      "INSERT INTO products (prod_name, price, category_id, image) VALUES ($1, $2, $3, $4) RETURNING *";
